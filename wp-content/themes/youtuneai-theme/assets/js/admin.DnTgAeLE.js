class YouTuneAIAdminDashboard {
      ...
      <div class="ytai-dashboard-wrapper">
        ...
        <div class="ytai-theme-selector">
          <select id="ytai-theme-select" class="ytai-select">
            ${Object.entries(this.themes).map(
      ([key, theme]) => `<option value="${key}" ${this.settings.theme === key ? "selected" : ""}>${theme.name}</option>`
    ).join("")}
          </select>
            ...
            <div class="ytai-setting-group">
              <label>
                ...
                <select id="ytai-refresh-rate">
                  <option value="2000" ${this.settings.autoRefresh === 2e3 ? "selected" : ""}>2s</option>
                  <option value="5000" ${this.settings.autoRefresh === 5e3 ? "selected" : ""}>5s</option>
                  <option value="10000" ${this.settings.autoRefresh === 1e4 ? "selected" : ""}>10s</option>
                </select>
              </label>
            </div>
  ...
  updateProgressBar() {
    ...
    statusBadge.querySelector(".ytai-status-dot");
  ...
  handleKeyboard(event) {
    if (this.bossMode.userSequence.length < this.bossMode.sequence.length) {
      ...
      const isCorrect = this.bossMode.userSequence.every(
        (key, index) => key === this.bossMode.sequence[index]
      );
      if (!isCorrect) {
  ...
  applyTheme() {
    ...
    this.container.className = this.container.className.replace(/ytai-theme-\w+/g, "").concat(` ytai-theme-${this.settings.theme}`);
  }
  toggleSettings() {
